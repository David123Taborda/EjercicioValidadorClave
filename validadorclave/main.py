    ...
    def _validar_longitud(self, clave):
        if len(clave) <= self._longitud_esperada:
            raise LongitudInvalidaError(
                "La clave debe tener una longitud de más de {} caracteres".format(self._longitud_esperada))
...
class ReglaValidacionGanimedes(ReglaValidacion):
    ...
    def contiene_caracter_especial(self, clave):
        ...
        if not any(c in especiales for c in clave):
            raise FaltaCaracterEspecialError("La clave debe contener al menos un carácter especial (@, _, #, $ o %)")
...
class ReglaValidacionCalisto(ReglaValidacion):
    ...
    def contiene_calisto(self, clave):
        ...
        if idx == -1:
            raise CalistoMalFormadoError("La clave debe contener la palabra calisto")

        palabra_original = clave[idx:idx + 7]
        mayusculas = sum(1 for c in palabra_original if c.isupper())

        if mayusculas < 2 or mayusculas == 7:
            raise CalistoMalFormadoError("La palabra calisto debe estar escrita con al menos dos letras en mayúscula")
